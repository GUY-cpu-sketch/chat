...
const chatColorSelect = document.getElementById("chatColor");
...
function playSound(name){ try{ if(sounds[name]) sounds[name].play().catch(()=>{}); } catch(e) {} }
...
if(chatForm){
  ...
  chatColorSelect?.addEventListener("change",(e)=>{
    ...
    socket.emit("setProfile",{color:myColor});
  });
  ...
  avatarInput?.addEventListener("change",(e)=>{
    ...
    if(file){
      ...
      reader.onload = function(evt){
        ...
        socket.emit("setProfile",{avatar:myAvatar});
      }
