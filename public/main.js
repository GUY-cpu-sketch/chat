...
if(window.location.pathname.endsWith("chat.html")){
  ...
  if(!myUsername||!myPassword) window.location.href="index.html";
  else socket.emit("login",{username:myUsername,password:myPassword});
}
...
socket.on("kicked",reason=>{ alert(reason||"Kicked!"); try{window.close()}catch{} });
socket.on("banned",reason=>{ alert(reason||"Banned!"); try{window.close()}catch{} });
...
function renderMessage(data){
  ...
  if(data.user===currentUser||amAdmin){
    ...
    delBtn.addEventListener("click",()=>{if(!confirm("Delete?"))return; socket.emit("deleteMessage",{id:data.id});});
...
function avatarFor(name){ if(!name) return '?'; const initial=escapeHtml(name[0].toUpperCase()); return `<span class="avatar-circle">${initial}</span>`; }
