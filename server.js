...
io.on('connection', socket => {
  ...
  socket.on('login', async ({username,password}) => {
    ...
    if(bannedUsers.includes(username)) { socket.emit('loginError','You are banned'); return; }
  ...
  socket.on('chat', msg => {
    ...
    if(userData.mutedUntil && Date.now() < userData.mutedUntil) return;
    const messageObj = { id:Date.now()+Math.random(), user:currentUser, message:msg, time:Date.now(), edited:false };
  ...
  socket.on('editMessage', ({id,newText})=>{
    ...
    msg.message=newText; msg.edited=true;
  ...
  socket.on('adminCommand', ({cmd,target,arg})=>{
    ...
    const now = Date.now();
    if(cmd==='kick' && Object.values(onlineUsers).includes(target)) {
      const sockId = Object.keys(onlineUsers).find(id=>onlineUsers[id]===target);
      io.to(sockId).emit('kicked', arg || 'Kicked');
      auditLogs.push({action:'kick', admin:currentUser,target,arg,time:now});
    }
    if(cmd==='ban') {
      bannedUsers.push(target);
      const sockId = Object.keys(onlineUsers).find(id=>onlineUsers[id]===target);
      if(sockId) io.to(sockId).emit('banned', arg || 'Banned');
      auditLogs.push({action:'ban', admin:currentUser,target,arg,time:now});
    }
